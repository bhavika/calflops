    ...
    for _ in range(input_shape[0]):
        ...
        for key in inputs.keys():
            if key == "input_ids":
                input_ids = inputs["input_ids"]
                pad_token = (
                    model_tokenizer.pad_token_id if model_tokenizer.pad_token_id else 0
                )
                input_ids = input_ids + ([pad_token] * padding_length)
                assert (
                    len(input_ids) == max_length
                ), "len(input_ids) must equal max_length"
                model_input_ids.append(input_ids)
            elif key == "attention_mask":
                attention_mask = [1] * origin_length
                attention_mask = attention_mask + ([0] * padding_length)
                ...
                ), "len(attention_mask) must equal max_length"
                model_attention_mask.append(attention_mask)
            elif key == "token_type_ids":
                token_type_ids = inputs["token_type_ids"]
                pad_token_segment_id = 0
                ...
                ), "len(token_type_ids) must equal max_length"
                model_token_type_ids.append(token_type_ids)
                ...
                for i in range(origin_length, max_length):
                    position_ids.append(i)
                assert (
                    len(position_ids) == max_length
                ), "len(position_ids) must equal max_length"
                model_position_ids.append(position_ids)
...
def get_module_flops(module, is_sparse=False):
    ...
    sum_flops = (
        module.__flops__
        * sum(p.count_nonzero().item() for p in module.parameters() if p.requires_grad)
        / sum(p.numel() for p in module.parameters() if p.requires_grad)
        if is_sparse
        else module.__flops__
    )
